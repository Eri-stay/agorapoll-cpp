import 'package:flutter/material.dart';

class AppColors {
  static const Color background = Color(0xFFfdfdfd); // Off-white
  static const Color textPrimary = Color(0xFF1A1A1A); // Dark charcoal
  static const Color textSecondary = Color(0xFF717171);
  static const Color lightGreyBg = Color(0xFFF0F0F0);
  static const Color snackBarGrey = Color(0xFF4A4A4A);
  static const Color accentGold = Color(0xFFB48A3A); // Gold
  static const Color mutedGold = Color(0xFFC9A66D); // Muted Gold
  static const Color darkGold = Color(0xFF5e4c2a); // Dark Gold (Brown)
  static const Color textFieldFill = Color(0xFFEBEDF1); // Light for text field
  static const Color cardBackground = Color(0xFFFAFAFA);
  static const Color cardOutline = Color(0xFFD0D0D0);
  static const Color redText = Color(0xFFA83232);
  static const Color error = Color(0xFFD64545); // Warm muted red
  static const Color errorLight = Color(0xFFF2B8B8); // Light red background
  static const Color warning = Color(0xFFc2a24c); //
  static const Color warningLight = Color(0xFFF6E3B3); // Soft for warnings
  static const Color charcoal = Color(0xFF1A1A1A);
}
